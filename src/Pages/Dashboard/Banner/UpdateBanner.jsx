...
import { getBannerById, postBanner, updateBanner } from "../../../api/AllApi";
...
export default function UpdateBanner() {
  ...
  const { data, isPending } = useQuery({
    queryKey: ["getBannerById", id],
    queryFn: () => getBannerById(id),
    refetchInterval: 1000,
  });

  const mutation = useMutation({
    mutationFn: (bannerData) => updateBanner(bannerData, id),

    onSuccess: (res) => {
      if (res.status === 200) {
        toast.success("Banner updated succesfully", { autoClose: 1000 });
        reset();
      }
    },
  });
    ...
    <div>
      <div className="bg-base-200 min-h-screen pt-10 px-5 md:px-0">
        ...
        <div className=" md:w-1/2 w-full  mx-auto py-5 bg-base-100 rounded-lg shadow-lg p-4 border border-success">
          <PreBackButton title="Update Banner" />
            ...
            <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl p-8">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                ...
                <div>
                  ...
                  <label className="flex items-center justify-center h-40 border-2 border-gray-900 border-dashed rounded-xl cursor-pointer overflow-hidden">
                    {preview ? (
